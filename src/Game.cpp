...
Game::Game(sf::VideoMode vid, const sf::String &title, sf::Uint32 style,
...
physics::HitboxManager &Game::getRealHitboxManager() { return m_hitboxManager; }
...
void Game::lost() {
  ...
  m_window.getRealTextManager().getRealTexts().insert(
      std::make_pair(0, std::make_shared<sf::Text>()));
  text = m_window.getRealTextManager().getRealTexts()[0];

  text->setString("You loose !\n\nPress enter to continue");
  text->setFont(
      *m_window.getRealRessources().getRealFonts()[graphics::FONT_MAIN]);
...
bool Game::updateLevel() {
  if (m_window.getRealEnemis().empty()) {
    std::shared_ptr<sf::Text> text;
    m_window.getRealTextManager().getRealTexts().insert(
        std::make_pair(1, std::make_shared<sf::Text>()));
...
int Game::update() {
        ...
        case sf::Keyboard::Return:
          if (m_over) {
            ...
            m_window.getRealCharacter().setPosition(sf::Vector2i(
                (int)(m_window.getSize().x / 2 - m_window.getRealCharacter()
                                                         .getSprite()
                                                         ->getGlobalBounds()
                                                         .width /
                                                     2),
  ...
  if (!m_over && !m_readyForNext) {
    m_window.getRealCharacter().update(m_clock.getElapsedTime(),
                                       sf::Vector2i(m_window.getSize()),
                                       m_window, m_hitboxManager);

    for (std::list<std::shared_ptr<Enemi>>::iterator it(
             m_window.getRealEnemis().begin());
         it != m_window.getRealEnemis().end(); it++)
      (*it)->update(m_clock.getElapsedTime(), sf::Vector2i(m_window.getSize()),
                    m_window, m_hitboxManager);
...
int Game::updateCollisions() {
  if (m_hitboxManager.isColliding(*m_window.getRealCharacter().getHitbox())) {
    ...
    for (std::map<int, std::shared_ptr<Missile>>::iterator it(
             m_window.getRealCharacter().getRealMissiles().begin());
         it != m_window.getRealCharacter().getRealMissiles().end(); it++) {
      for (std::list<std::shared_ptr<Enemi>>::iterator it2(
               m_window.getRealEnemis().begin());
           it2 != m_window.getRealEnemis().end(); it2++) {
        if (m_hitboxManager.areColliding(*it->second->getRealHitbox(),
                                         *(*it2)->getRealHitbox())) {
          (*it2)->clean(std::make_shared<graphics::SpriteManager>(
                            m_window.getRealSpriteManager()),
                        m_window);
          m_window.getRealEnemis().erase(it2);

          it->second->clean(std::make_shared<graphics::SpriteManager>(
                                m_window.getRealSpriteManager()),
                            m_window);
...
void Game::newLevel(const int &nbW, const int &nbH) {
  for (std::list<std::shared_ptr<Enemi>>::iterator it(
           m_window.getRealEnemis().begin());
       it != m_window.getRealEnemis().end(); it++) {
    (*it)->clean(std::make_shared<graphics::SpriteManager>(
                     m_window.getRealSpriteManager()),
                 m_window);
  }
  ...
  for (int i(0); i < nbH; i++) {
    for (int j(0); j < nbW; j++) {
      m_window.getRealEnemis().push_back(std::make_shared<Enemi>(
          m_hitboxManager,
          sf::Rect<int>(wgap * j, i * 80, (1024 / 2) + 64, 600), m_window));
