...
Entity::Entity(physics::HitboxManager &hitboxManager)
  ...
  m_hitboxManager.getRealHitboxes().push_back(m_hitbox);
}

Entity::Entity(const Entity &entity) : m_hitboxManager(getHitboxManager()) {
...
sf::Vector2i Entity::getPosition() const { return m_position; }
...
sf::Vector2f Entity::getSpeed() const { return m_speed; }
...
sf::Vector2i Entity::move(const int &dx, const int &dy,
                          const sf::Rect<int> &wall) {
  if ((wall.left == -1 && wall.top == -1) ||
      (m_hitbox->getRealRect().left + dx >= wall.left &&
       m_hitbox->getRealRect().left + m_hitbox->getRealRect().width + dx <=
           wall.left + wall.width &&
       m_hitbox->getRealRect().top + dy >= wall.top &&
       m_hitbox->getRealRect().top + m_hitbox->getRealRect().height + dy <=
    ...
    if (m_hitbox->getRealRect().left + m_hitbox->getRealRect().width + dx >
            wall.left + wall.width &&
        dx > 0) {
      // m_position.x += (wall.left + wall.width -m_hitbox->getRealRect().width
      // - m_hitbox->getRealRect().left);
      m_hitbox->getRealRect().left =
          wall.left + wall.width - m_hitbox->getRealRect().width;
      m_position.x = m_hitbox->getRealRect().left + gapX;
    }
    ...
    if (m_hitbox->getRealRect().top + m_hitbox->getRealRect().height + dy >
            wall.top + wall.height &&
        dy > 0) {
      // m_position.y += (wall.top + wall.height -
      // m_hitbox->getRealRect().height - m_hitbox->getRealRect().top);
      m_hitbox->getRealRect().top =
          wall.top + wall.height - m_hitbox->getRealRect().height;
      m_position.y = m_hitbox->getRealRect().top + gapY;
    }
  }
...
std::shared_ptr<physics::Hitbox> Entity::getHitbox() const { return m_hitbox; }

std::shared_ptr<physics::Hitbox> &Entity::getRealHitbox() { return m_hitbox; }

std::shared_ptr<sf::Sprite> Entity::getSprite() const { return m_sprite; }

std::shared_ptr<sf::Sprite> &Entity::getRealSprite() { return m_sprite; }
...
int Entity::print(graphics::GameWindow &win) {
  getRealSprite()->setPosition(sf::Vector2f(getPosition()));
  win.getRealLayers()[graphics::WINDOW_LAYER_ITEM]->addSprite(
      m_hitbox->getId());
...
int Entity::erase(graphics::GameWindow &win) {
  ...
  it(win.getRealLayers()[graphics::WINDOW_LAYER_ITEM]->getRealSprites().begin());
  while (it !=
  win.getRealLayers()[graphics::WINDOW_LAYER_ITEM]->getRealSprites().end())
  */

  for (unsigned int i(0); i < win.getRealLayers()[graphics::WINDOW_LAYER_ITEM]
                                  ->getRealSprites()
                                  .size();
       i++) {
    if (win.getRealLayers()[graphics::WINDOW_LAYER_ITEM]->getRealSprites()[i] ==
        m_hitbox->getId()) {
      win.getRealLayers()[graphics::WINDOW_LAYER_ITEM]->getRealSprites().erase(
          win.getRealLayers()[graphics::WINDOW_LAYER_ITEM]
              ->getRealSprites()
              .begin() +
          i);
    }
...
void Entity::update(const sf::Time &t, const sf::Vector2i &wall,
  ...
  if (m_speed.x != 0 || m_speed.y != 0) m_lastUpdate += t;
  if (m_lastUpdate.asSeconds() * m_speed.x >= 1 ||
      m_lastUpdate.asSeconds() * m_speed.y >= 1 ||
      m_lastUpdate.asSeconds() * m_speed.x <= -1 ||
      m_lastUpdate.asSeconds() * m_speed.y <= -1) {
    move((int)(m_lastUpdate.asSeconds() * m_speed.x),
         (int)(m_lastUpdate.asSeconds() * m_speed.y),
...
void Entity::clean(std::shared_ptr<graphics::SpriteManager> spManager,
                   graphics::GameWindow &win) {
  // Clean the sprite
  spManager->getRealSprites().erase(
      spManager->getRealSprites().find(m_hitbox->getId()));

  erase(win);

  // Clean the hitbox
  for (std::deque<std::shared_ptr<physics::Hitbox>>::iterator it(
           m_hitboxManager.getRealHitboxes().begin());
